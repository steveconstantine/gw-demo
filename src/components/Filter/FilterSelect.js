import React, { Component } from 'react';
import { Box, Button, Flyout } from 'gestalt';

import Select from 'react-select';
import { colors } from 'react-select/lib/theme';
const stateOptions = [
  { key: 'angular', label: 'Angular', value: 'angular' },
  { key: 'css', label: 'CSS', value: 'css' },
  { key: 'design', label: 'Graphic Design', value: 'design' },
  { key: 'ember', label: 'Ember', value: 'ember' },
  { key: 'html', label: 'HTML', value: 'html' },
  { key: 'ia', label: 'Information Architecture', value: 'ia' },
  { key: 'javascript', label: 'Javascript', value: 'javascript' },
  { key: 'mech', label: 'Mechanical Engineering', value: 'mech' },
  { key: 'meteor', label: 'Meteor', value: 'meteor' },
  { key: 'node', label: 'NodeJS', value: 'node' },
  { key: 'plumbing', label: 'Plumbing', value: 'plumbing' },
  { key: 'python', label: 'Python', value: 'python' },
  { key: 'rails', label: 'Rails', value: 'rails' },
  { key: 'react', label: 'React', value: 'react' },
  { key: 'repair', label: 'Kitchen Repair', value: 'repair' },
  { key: 'ruby', label: 'Ruby', value: 'ruby' },
  { key: 'ui', label: 'UI Design', value: 'ui' },
  { key: 'ux', label: 'User Experience', value: 'ux' },
]
const selectStyles = {
  control: provided => ({ ...provided, minWidth: 240, margin: 8 }),
  menu: () => ({ boxShadow: 'inset 0 1px 0 rgba(0, 0, 0, 0.1)' }),
};

export default class FilterSelect extends Component {
  state = { isOpen: false, value: undefined };
  toggleOpen = () => {
    this.setState(state => ({ isOpen: !state.isOpen }));
  };
  onSelectChange = value => {
    this.toggleOpen();
    this.setState({ value });
  };
  render() {
    const { isOpen, value } = this.state;
    return (
 <Box>
        <div
          style={{ display: "inline-block" }}
          ref={c => {
            this.anchor = c;
          }}
        >
          <Button
            accessibilityExpanded={!!this.state.isOpen}
            accessibilityHaspopup
            onClick={this.toggleOpen}
            text="Help"
          />
        </div>
        {this.state.isOpen &&
          <Flyout
            anchor={this.anchor}
            idealDirection="up"
            size="md"
          >
            <Box padding={3}>
                <Select
          autoFocus
          components={{ DropdownIndicator, IndicatorSeparator: null }}
          controlShouldRenderValue={false}
          hideSelectedOptions={false}
          isClearable={false}
          menuIsOpen
          onChange={this.onSelectChange}
          options={stateOptions}
          placeholder="Search..."
          styles={selectStyles}
          tabSelectsValue={false}
          value={value}
        />
            </Box>
          </Flyout>}
      </Box>
    );
  }
}

// styled components

const Menu = props => {
  const shadow = colors.neutral10a;
  return (
    <div
      css={{
        backgroundColor: 'white',
        borderRadius: 4,
        boxShadow: `0 0 0 1px ${shadow}, 0 4px 11px ${shadow}`,
        marginTop: 8,
        position: 'absolute',
        zIndex: 2,
      }}
      {...props}
    />
  );
};
const Blanket = props => (
  <div
    css={{
      bottom: 0,
      left: 0,
      top: 0,
      right: 0,
      position: 'fixed',
      zIndex: 1,
    }}
    {...props}
  />
);
const Dropdown = ({ children, isOpen, target, onClose }) => (
  <div css={{ position: 'static' }}>
    {target}
    {isOpen ? <Menu>{children}</Menu> : null}
    {isOpen ? <Blanket onClick={onClose} /> : null}
  </div>
);
const Svg = p => (
  <svg
    width="24"
    height="24"
    viewBox="0 0 24 24"
    focusable="false"
    role="presentation"
    {...p}
  />
);
const DropdownIndicator = () => (
  <div css={{ color: colors.neutral20, height: 24, width: 32 }}>
    <Svg>
      <path
        d="M16.436 15.085l3.94 4.01a1 1 0 0 1-1.425 1.402l-3.938-4.006a7.5 7.5 0 1 1 1.423-1.406zM10.5 16a5.5 5.5 0 1 0 0-11 5.5 5.5 0 0 0 0 11z"
        fill="currentColor"
        fillRule="evenodd"
      />
    </Svg>
  </div>
);
const ChevronDown = () => (
  <Svg style={{ marginRight: -6 }}>
    <path
      d="M8.292 10.293a1.009 1.009 0 0 0 0 1.419l2.939 2.965c.218.215.5.322.779.322s.556-.107.769-.322l2.93-2.955a1.01 1.01 0 0 0 0-1.419.987.987 0 0 0-1.406 0l-2.298 2.317-2.307-2.327a.99.99 0 0 0-1.406 0z"
      fill="currentColor"
      fillRule="evenodd"
    />
  </Svg>
 );